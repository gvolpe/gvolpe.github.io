...
(function() {
  ...
  function initializeCodeFences() {
    ...
    highlights.forEach((highlight) => {
      ...
      if (code) {
        ...
        const hlClass = Array.from(highlight.classList);

        if (hlClass && hlClass.includes("unison-lang")) {
          highlight.setAttribute("data-lang", "unison");
        } else if (hlClass && hlClass.includes("dhall-lang")) {
          highlight.setAttribute("data-lang", "dhall");
        } else if (langClass) {
    ...
    if (results.length === 0) {
      searchResults.innerHTML = `
        <div class="search-no-results">
          No article found for "${query}". Try different keywords or browse by tags.
        </div>
      `;
    } else {
  ...
  function handleSearchResultClick(url) {
    ...
    if (window.spa && url.startsWith("/blog/")) {
  ...
  class LofiCodeSPA {
    ...
    detectPageType() {
      ...
      const isPostURL = window.location.pathname.startsWith('/blog/');
    ...
    async loadPagesData() {
      try {
        const response = await fetch("/talks/pages.json");
        if (response.ok) {
    ...
    navigateToPost(href) {
      const slug = href.split("/blog/")[1].replace("/", "");
